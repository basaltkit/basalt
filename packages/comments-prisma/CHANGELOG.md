...
- Initial release. Prisma-backed implementation of the @basaltkit/comments `CommentStore`, on a Prisma client (PostgreSQL/MySQL); ships a reference `schema.prisma`. `prismaCommentsStore(prisma)` returns the store named to drop straight into `commentsPlugin`. The production counterpart to `@basaltkit/comments-sqlite`.
