...
- Initial release. Durable, SQLite-backed implementation of the @basaltkit/activity `ActivityStore` (activity feed), on Node's built-in `node:sqlite`, with zero external dependencies. `sqliteActivityStore(location)` returns the store named to drop straight into `activityPlugin`. The single-node counterpart to `@basaltkit/activity-prisma`.
