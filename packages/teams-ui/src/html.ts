...
// teamsPageCsp) come from the exact same source text. Server-side state is
// embedded with scriptJson (headers/roles may carry per-tenant data — a
// crafted value cannot terminate the script block); API responses render
// through esc(), whose charset includes quotes for attribute positions.
const pageScript = (apiBase: string, roles: string[], headers: Record<string, string>): string => `
const API = ${scriptJson(apiBase)};
const ROLES = ${scriptJson(roles)};
...
/**
 * The Content-Security-Policy matching {@link teamsPageHtml}: everything locked
 * down, the page's inline script allowed by sha256 hash. Served by default from
 * `teamsUiRoutes` so the page works under `securityPlugin` without weakening
 * the app-wide policy.
 */
export function teamsPageCsp(options: TeamsPageOptions = {}): string {
  const apiBase = options.apiBase ?? ''
  return pageCsp({
    scripts: [pageScript(apiBase, options.roles ?? ['owner', 'admin', 'member'], options.headers ?? {})],
...
export function teamsPageHtml(options: TeamsPageOptions = {}): string {
  const apiBase = options.apiBase ?? ''
  const title = escapeHtml(options.title ?? 'Team')
...
<div class="card">
  <form id="invite">
    ...
    <label>Role<select name="role">${roles.map((r) => `<option>${escapeHtml(r)}</option>`).join('')}</select></label>
...
<script>${pageScript(apiBase, roles, headers)}</script>
</body>
</html>`
}
