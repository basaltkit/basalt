...
- bbfb7ff: New package: `@basaltkit/teams-ui` — a management page for `@basaltkit/teams`.

  `teamsUiRoutes({ path?, apiBase?, title?, roles?, headers? })` serves a self-contained, dependency-free HTML page at `GET /team/ui` (requires a logged-in user) that drives `@basaltkit/teams`' routes: invite a member (`POST /team/invites`), list and revoke pending invitations, and list members with a role dropdown (`PATCH /team/members/:userId`) and remove (`DELETE`). It fetches same-origin; the optional `headers` inject header-based tenancy (`x-tenant-id`), while subdomain apps need nothing. `teamsPageHtml(...)` returns the HTML string for custom serving. Tested (the generated page and the end-to-end HTTP flow against the real team routes with tenancy + auth).
