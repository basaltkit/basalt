 ...
 * directly (no SDK dependency) so the same message handler drives every
 * transport (HTTP, stdio). The methods implemented by the core dispatcher are the
 * stable set: `initialize`, `tools/list`, `tools/call`, `ping`, and — when the
 * server registers them — `resources/list`, `resources/read`, `prompts/list`,
 * `prompts/get`.
 */
