...
```bash
pnpm add @basaltkit/api-keys-ui
```

Depends on `@basaltkit/core` and `@basaltkit/fastify`. Requires your app to mount the API key routes from `@basaltkit/auth` (`apiKeysPlugin` + `apiKeyRoutes`).
...
page's inline script allowed only by sha256 hash (exported as `apiKeysPageCsp`). It
...
- **`@basaltkit/auth`** — provides the API key routes this page consumes (`apiKeysPlugin` + `apiKeyRoutes`).
- **`@basaltkit/permissions`** — add a *guard* to the page route if you want to restrict who can see it.
