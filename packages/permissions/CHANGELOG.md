...
- b0cc59f: Role catalogue for per-tenant roles (BK-016). A role's permissions were looked up only in the scope where the role is held, so a catalogue granted once in `GLOBAL_SCOPE` never applied to roles `@basaltkit/teams` assigns per tenant — apps had to copy it into every tenant.
...
- fb85c40: Security hardening (B04):
...
- 30abb78: `meta.audience`: a permission is a capability, not a surface.
