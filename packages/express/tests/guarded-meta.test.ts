...
import { expressPlugin } from '../src/index.js'
...
describe('express: boot fails loud when security meta has no enforcing guard', () => {
  it('meta.auth with no enforcing plugin → boot rejects (fail closed, before traffic)', async () => {
    await expect(
      createApp({ plugins: [expressPlugin({ routes: [protectedRoute] })] }).boot(),
    ).rejects.toBeInstanceOf(UnguardedRouteMetaError)
  })

  it('boots when the enforcing plugin claimed the key', async () => {
    const app = await createApp({ plugins: [claimingPlugin, expressPlugin({ routes: [protectedRoute] })] }).boot()
    await app.shutdown()
  })

  it('allowUnguardedMeta waives the check for edge-auth deployments', async () => {
    const app = await createApp({
      plugins: [expressPlugin({ routes: [protectedRoute], allowUnguardedMeta: ['auth'] })],
    }).boot()
    await app.shutdown()
  })
})
