...
- 7363b76: `make:service` no longer generates a file that does not compile (BK-020).
...
- fb85c40: Security: `basalt make:resource` (and `make:routes`/`make:repository`/`make:test`) now generates secure-by-default code. Previously the generated CRUD was anonymous and shared across tenants, and `make:resource` auto-wired it into `src/app.ts`.
...
- 36ab1a1: The generated repository can be typed against any Prisma client, and
