...
/**
 * Sibling of the Fastify and Express suites — same guarantees, same adapter.
 *
...
async function boot() {
  seen.length = 0
  app = await createApp({
    plugins: [honoPlugin({ routes, onError: (r) => void seen.push(r) })] as never,
  }).boot()
...
describe('HTTP error reporting on Hono', () => {
  it('reports a 500 from a throwing handler', async () => {
    ...
    expect(seen[0]!.url).toContain('/boom')
  ...
  it('reports a deliberate 4xx thrown as HttpError', async () => {
    await boot()
    expect((await call('/teapot')).status).toBe(418)
    expect(seen).toHaveLength(1)
