...
> **Status: 0.29.0 — 67 packages, all published to npm.** The architecture is
> stable and every package is tested, but APIs may still change before 1.0.
> In-memory stores are the dev default; durable backends are landing — auth,
> teams, subscriptions, comments, audit, activity and notifications now persist
> via SQLite (`*-sqlite`, single-node, zero-dep) or Prisma (`*-prisma`,
> Postgres/MySQL). See
