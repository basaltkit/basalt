...
> page. **Every section of this RFC has now shipped** — the design-only backlog is
> cleared; what remains is ongoing depth (more upgrade codemods, more make:* kinds,
> billing analytics), not missing pillars.
>
...
### 12.1 `@basaltkit/storage`  `[✅ shipped]`
```ts
...
```
Drivers: MinIO/S3 (same driver, S3-compatible), Local, Azure Blob, GCS — all passing the same conformance suite. Per-tenant isolation via prefix (default) or a dedicated bucket (config). Image processing via a fluent, engine-neutral `disk.image(...)` pipeline (`.resize().webp().save()`); the `sharp` engine ships as the opt-in satellite `@basaltkit/image-sharp` (see §9.1), so apps that never touch images carry no native dependency — run it inside a job to keep it off the request path. Direct browser→storage upload with pre-signed URLs generated by the backend.

### 12.2 `@basaltkit/queue` + `@basaltkit/jobs`  `[✅ shipped — jobs is part of queue]`
```ts
...
### 12.8 `@basaltkit/mailer`  `[✅ shipped]`
Drivers for SMTP, Resend, SES and Mailgun + a `log` driver (dev) and a `memory` driver (test). Typed declarative mails (Zod-validated) render text/HTML from any engine — plain literals, **MJML**, or **React Email** — inside `html()`; a shared `layout` hook wraps every body for per-tenant branding; sending goes through the queue when wired. `mail:preview` serves a browser dev server (`createMailPreviewServer`) that renders every registered mail with its sample data, reusing the real schema + layout.
...
## 14. Documentation (basalt.dev)  `[✅ shipped]`
...
Getting Started carries an **Open in StackBlitz** button that boots the runnable `apps/playground` in a WebContainer (a reusable `<StackBlitz>` theme component). Search is **Algolia DocSearch** when `ALGOLIA_APP_ID`/`ALGOLIA_API_KEY`/`ALGOLIA_INDEX` are set, falling back to VitePress's local index otherwise. The **API Reference** is generated by typedoc (`docs:api`, markdown via `typedoc-plugin-markdown` + `typedoc-vitepress-theme`) into a VitePress-native sidebar.
